...
const AdminResults = () => {
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [debouncedSearchTerm, filterScore])
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/admin/results?page=${currentPage}&limit=10&filter=${filterScore}&search=${debouncedSearchTerm}`
      )
  ...
  useEffect(() => {
    fetchResults()
  }, [currentPage, filterScore, debouncedSearchTerm])
  ...
  // REMOVED: Client-side filtering and pagination - now using server-side only
  // The results are already paginated from the server, so we use them directly
  // Display results directly without additional slicing
        ...
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {/* Use results directly from API (already paginated) */}
                {results.length > 0 ? (
                  results.map((result) => (
                    <tr key={result.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
